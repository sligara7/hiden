...
class MASsoftClient:
    ...
    def open_experiment(self, file_name=None):
        """Open and associate an experiment file on the command socket.

        This is the primary method for opening experiments (used in IPython workflow).
        Also available as open_experiment_commands().
        """
    ...
    # Alias for use by the IOC (cap2.py)
    open_experiment_commands = open_experiment

    def open_experiment_data(self, file_name=None):
        """Open and associate an experiment file on the data socket."""
        if file_name is None:
    ...
    def open_experiment_status(self, file_name=None):
        """Open and associate an experiment file on the status socket."""
        if file_name is None:
            full_path = self.query_filename()
        else:
            if isinstance(file_name, (list, tuple)):
    ...
    def get_data(self, view=1):
        """Retrieve scan data via the data socket."""
        if not self.current_file:
            raise RuntimeError("No file opened.")
        headers, _path = self.get_legends(view=view)
        data = []
        while True:
    ...
    def get_legends(self, view=1):
        """Retrieve column legends via the command socket.

        Returns (legend_list, path).
        """
...
# data = client.get_data(); legends, path = client.get_legends()
